...
```{r}
maxTime <- max(max(sensorAcq$time),max(LosData$time))
```
...
targets <- dplyr::bind_rows(
  dplyr::select(.data = sensorAcq,
                targetId),
  dplyr::select(.data = LosData,
                targetId)
) %>%
  dplyr::distinct(targetId)

```

```{r}

sensors <- dplyr::distinct(entitySensorMapping$SensorToEntityId,sensorId)
